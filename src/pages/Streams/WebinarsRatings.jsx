...
const WebinarsRatings = () => {
  ...
  useEffect(() => {
    document.title = `Webinar Visited | AP Education`;
    const getWebinarUsersRequest = async () => {
      try {
        setIsLoading(isLoading => (isLoading = true));
        const usersToSet = await axios.get('/webinarusers/rating');
      ...
      <TeacherSpeakingDBTable>
        ...
        <tbody>
          {users
            ...
            .sort(
              (a, b) =>
                new Date(changeDateFormat(b.visitedTime[b.visitedTime.length - 1])) -
                new Date(changeDateFormat(a.visitedTime[a.visitedTime.length - 1]))
            )
            .map((user, i) => (
...
export default WebinarsRatings;
