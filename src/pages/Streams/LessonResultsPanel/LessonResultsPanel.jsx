...
import { getKahoots } from 'pages/Videochat/utils/api/getKahoots';
import {
  AdminFormBtn,
  AdminInput,
  AdminInputNote,
...
const LessonResultsPanel = () => {
  ...
  const handleFindLessons = async () => {
    try {
      const response = await axios.get(
        `/lesson-results/findOne/${selectedCourse}/${selectedDate.toLocaleDateString(
          'uk-UA'
        )}`
      );
  ...
  const handleFetchLessonPoints = async lessonId => {
    const response = await axios.get(`/lesson-results/points/${lessonId}`);
